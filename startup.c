...
void 
start() 					/*call this every new level*/
{
	void            nextpiece();
	extern char     c[];
	int             i, x, y, z, ch;
	...
	for (i = 0; i < v; i++) {
		do {
			...
			y = rx(14, 4);
			ch = mvwinch(w, y, x);
			if ((mvwinch(w, y + 3, x) == c[z] || mvwinch(w, y - 3, x) == c[z])
			    || (mvwinch(w, y, x + 3) == c[z] || mvwinch(w, y, x - 3) == c[z])) /* can't have 4 in a row or col to start */
				ch = 0;
