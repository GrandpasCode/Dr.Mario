...
void loop()
{
    ...
    {                           /*till no bugs left */
        ...
        piece++;                /*used a piece */
        if (sc)
        {                       /*figure out score. if sc=0 we didn't remove any bugs */
            int sco = sp;
            if (sc > 6)
                sc = 6;
            for (; sc > 1; --sc)
...
int mvcheck(int y1, int x1)     /*see if we can move or not */
{
    int a = BLANK, b = BLANK;
...
void change(int dir)            /*rotate piece */
{
    ...
    if (p2->x == 8 && cp & 1)
    {                           /*if we're on the right wall vertically
